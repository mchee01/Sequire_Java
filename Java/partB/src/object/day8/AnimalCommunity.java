...
public class AnimalCommunity {
    ...
    //			ㄴ Member 클래스 isAdopt 메소드에서 실행. -> intanceof 연산 사용		
		...
		// StringBuilder 는 가변 객체 (문자열 변경 가능.문자열 추가 연산 결과를 동일 객체에 저장) => 문자열 연결연산에 사용
		sb.append(" 회원님 \n");				//message += " 회원님 \n";
		boolean result;
		for(int i=0;i<animals.length;i++) {
			if(animals[i]==null) 
			...
			String temp = member.isAdopt(animals[i]) ? "분양 가능합니다.\n":"분양 자격이 안됩니다.\n";
						//   조건식 ? "참일때 대입되는 문자열" : "거짓일 때 대입되는 문자열"
			sb.append(temp);

		}
		System.out.println();
		System.out.println(sb);  //StringBuilder 타입 객체는 println 의 인자로 쓸때 toString 생략 가능
